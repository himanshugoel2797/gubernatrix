...
void igfx_gmbus_disable_writeprot(igfx_driver_t *driver) {
...
void igfx_gmbus_enable_writeprot(igfx_driver_t *driver) {
  uint32_t val = igfx_read32(driver, driver->display_mmio_base + IGFX_GMBUS1);
  igfx_write32(driver, driver->display_mmio_base + IGFX_GMBUS1, val | (1 << 31));
}

void igfx_gmbus_reset(igfx_driver_t *driver) {
...
void igfx_gmbus_wait(igfx_driver_t *driver) {
  while (!(igfx_read32(driver, driver->display_mmio_base + IGFX_GMBUS2) & (1 << 11)))
    ;
}

void igfx_gmbus_stoptransaction(igfx_driver_t *driver) {
  igfx_write32(driver, driver->display_mmio_base + IGFX_GMBUS1, (1 << 30) | (1 << 27));
}

void igfx_gmbus_read(igfx_driver_t *driver, uint32_t disp_idx, uint8_t offset,
