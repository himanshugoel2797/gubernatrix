...
// Handle port status detection
// Implement display enable/disable
// Implement modeset
...
bool igfx_display_isconnected(igfx_driver_t *driver, uint32_t idx) {
  if (driver->architecture == IGFX_CHERRYTRAIL && driver->display_count > idx) {
    if (idx == 0)
...
void igfx_display_enable_port(igfx_driver_t *driver, uint32_t idx) {
  if (driver->architecture == IGFX_CHERRYTRAIL) {
    if (idx == 0) {
      ...
      igfx_write32(driver, driver->display_mmio_base + IGFX_CHV_HDMID,
                   IGFX_CHV_HDMID_ENABLE | IGFX_CHV_HDMID_AUDIO_OUT |
...
void igfx_display_pipe_setup(igfx_driver_t *driver, uint32_t idx) {
  if (driver->architecture ==
      IGFX_CHERRYTRAIL) { // port D only connects to PIPE C
    if (idx == 0) {       // Port D is only connected to PIPE C
      // Setup PIPE C
  ...
  for (uint32_t i = 0; i < driver->display_count; i++) {
    ...
    if (disp_connected &&
      ...
      igfx_display_pipe_setup(driver, i);
               ...
               driver->displays[i].connected) { // display just disconnected
